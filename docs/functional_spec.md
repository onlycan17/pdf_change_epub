...
PDF에서 추출한 텍스트를 분석하여 마크다운 형식으로 변환합니다. 무료 사용자도 로그인 없이 사용 가능합니다.
...
4. 무료 사용자는 기본 변환만 가능, 프리미엄 사용자는 고급 옵션 사용 가능
...
사용자가 구글 계정을 통해 서비스에 로그인할 수 있도록 합니다. 무료 기능은 로그인 없이 사용 가능하며, 프리미엄 기능 사용 시에만 로그인이 요구됩니다.

#### 동작 방식

1. **무료 사용자**: 로그인 없이 기본 기능을 즉시 사용 가능
2. **프리미엄 기능 선택 시**: 자동으로 로그인 페이지로 리다이렉트
3. 사용자가 "구글로 로그인" 버튼을 클릭합니다.
4. Supabase Auth를 통해 구글 OAuth 인증을 수행합니다.
5. 인증 성공 시 사용자 정보를 저장하고 세션을 생성합니다.
6. 이전에 시도한 프리미엄 기능으로 자동 복귀
