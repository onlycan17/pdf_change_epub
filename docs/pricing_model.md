...
- (현재) 무료 플랜만 운영: 25MB
- 결제/구독 기능은 추후 도입 예정이며, 현재는 광고/후원 기반으로 운영합니다.
- 결제 기능 재도입 방향은 `docs/monetization_free_only.md`를 참고하세요.

연간 플랜은 월간 가격의 12개월 합계에서 10%를 공제한 가격으로 계산하며, 가격은 다음과 같습니다.

- 월간: `9,900원`
- 연간: `9,900원 × 12 × (1 - 0.10) = 106,920원`

프론트엔드/백엔드 플랜 정책은 `app/services/subscription_plans.py`의 상수로 단일 소스처럼 관리하되,
현재는 `app/api/v1/conversion.py`에서 무료 한도(25MB)로 고정하여 동작합니다.
