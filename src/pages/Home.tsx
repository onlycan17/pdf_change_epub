...
import { FileUpload, UsageIndicator } from '../components/common'
...
const Home: React.FC = () => {
  ...
  const checkUserStatus = async () => {
    try {
      ...
      if (user) {
        // 실제로는 Supabase에서 사용자 정보를 가져와야 함
        // 여기서는 임시로 설정
        setIsPremium(false) // 실제로는 DB에서 확인
        setDailyUsage(2) // 실제로는 DB에서 확인
      }
    } catch (error) {
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-12">
          ...
          <div className="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-lg shadow-sm p-6 border-2 border-yellow-200">
            ...
            <p className="text-2xl font-bold text-gray-900">$9.99<span className="text-sm font-normal">/월</span></p>
